...
class Category extends \yii\db\ActiveRecord
{
   ...
   /*public function upload()
    ...
    public function getMainCategory(){
        return $this->find()->select(['id','name','image'])->where(['status'=>$this::STATUS_ACTIVE,'level'=>$this::LEVEL_MAIN])->andWhere(['type'=>Category::TYPE_AD_CATEGORY])->all();
    ...
    //     return $this->hasMany(Category::className(), ['parent_id' => 'id'])->andOnCondition(['status' => Category::STATUS_ACTIVE])->from(['subCategory' => Category::tableName()])->select(['id','name','parent_id']);
